...
def reflect_boundary(agents):
    ...
    agents[mask_top, 2] = (
        np.pi - agents[mask_top, 2] + np.random.uniform(-math.pi / 4, math.pi / 4, np.sum(mask_top))
    )
...
#@jit#@jit
def rotate_towards_sensor_simple(agents, sensor_values, sensors_angles, AGENT_NUMBER=AGENT_NUMBER, ROTATION_SPEED=ROTATION_SPEED):
