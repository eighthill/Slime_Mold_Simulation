...
class PheromoneArray:
    def __init__(self, x_len=1500, y_len=1500, fading=0.5, pheromone_value=50):
        self.world = np.zeros((x_len, y_len), dtype=int)
        self.fading = fading
        self.pheromone_value = np.full_like(self.world, pheromone_value, dtype=float)
        
    #def decay_pheromones(self):
    #    self.world = (self.world * (1 - SlimeConfig.evaporation_speed)).astype(int)

    def update_pheromone(self, Agents):
        self.world = (self.world * self.fading).astype(int)
        for agent in Agents:
            x, y = agent["int_x_pos"], agent["int_y_pos"]
            self.world[x, y] = self.pheromone_value[x, y]


# the agent class creates a list with one dictionary for each agent

class Agent:
    def __init__(self, array, num_agents=5000, sensor_angle=0.33, radius=0.2, speed=0.06):
