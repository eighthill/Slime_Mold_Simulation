...
class SimulationGUI(app.Canvas):
    ...
    def initialize_agents(self):
        ...
        agent_params = Agent.initialize_agents(center_x=center_x, center_y=center_y, num_agents=SlimeConfig.num_agents, radius=250, move_speed=SlimeConfig.move_speed, world_x=self.view.size[0], world_y=self.view.size[1])
        self.agents_data = [Agent(x=param['x'], y=param['y'], movement_angle=param['movement_angle'], pheromone_array=self.pheromone, move_speed=param['move_speed'], turn_speed=SlimeConfig.turn_speed, sensor_distances=SlimeConfig.sensor_distances, sensor_angles=SlimeConfig.sensor_angles, random_move=random.choice([True, False])) for param in agent_params]
