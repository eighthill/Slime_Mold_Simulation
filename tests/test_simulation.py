...
def test_agent_init():
    ...
    assert agent.agenten.shape == (AGENT_NUMBER, 3), f"Expected agent.agenten.shape to be {(AGENT_NUMBER, 3)}, but got {agent.agenten.shape}"
...
def test_get_sensors():
    ...
    assert sensors_angles.shape == (AGENT_NUMBER, 3), f"Expected 'sensors_angles' to have shape {(AGENT_NUMBER, 3)}, but got {sensors_angles.shape}."
...
def test_reflect_boundary():
    ...
    assert np.all(agents_reflected[:, 0] < HEIGHT), f"Expected all x positions to be < {HEIGHT} after reflection, but some are not."
    assert np.all(agents_reflected[:, 1] >= 0), "Expected all y positions to be >= 0 after reflection, but some are not."
    assert np.all(agents_reflected[:, 1] < WIDTH), f"Expected all y positions to be < {WIDTH} after reflection, but some are not."
...
def test_move():
    ...
    assert np.all(agents_moved[:, 0] < HEIGHT), f"After moving, expected all agents' x positions to be < {HEIGHT}, but some exceed this boundary."
    assert np.all(agents_moved[:, 1] >= 0), "After moving, expected all agents' y positions to be >= 0, but some are outside the boundary."
    assert np.all(agents_moved[:, 1] < WIDTH), f"After moving, expected all agents' y positions to be < {WIDTH}, but some exceed this boundary."
...
def test_main():
    ...
    assert np.all(agents[:, 0] < HEIGHT), f"Expected all agents' x positions to be < {HEIGHT}, ensuring they're within the vertical boundaries."
    assert np.all(agents[:, 1] >= 0), "Expected all agents' y positions to be >= 0, ensuring they're within the horizontal boundaries."
    assert np.all(agents[:, 1] < WIDTH), f"Expected all agents' y positions to be < {WIDTH}, ensuring they're within the horizontal boundaries."
